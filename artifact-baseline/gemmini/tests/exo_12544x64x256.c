...
void matmul6( matmul_Context *ctxt, const float* scale, bool act, const int8_t* A, const int8_t* B, int8_t* C ) {
...
#define i_stride 12544 / io_max / i_max
#define io_stride 12544 / io_max


int res_rows = 16 * 4 * i_max;
...
for (int_fast32_t io = 0; io < io_max; io++) { // Orig: 98
  for (int_fast32_t jo = 0; jo < jo_max; jo++) { // 1
    for (int_fast32_t i = 0; i < i_max; i++) { // Orig: 8
      for (int_fast32_t j = 0; j < j_max; j++) { // 1
        ...
        for (int_fast32_t ko = 0; ko < ko_max; ko++) { // Orig: 4
    ...
    gemmini_extended_compute_preloaded((uint32_t)(&*(int8_t*)((uint64_t)( ((uint32_t)((uint64_t)a)) + ((i) * (4096) + (ko) * (1024) + (3) * (256))/16))), ~((uint32_t)0), (16), (16), 16, 16);
        ...
        gemmini_extended_mvout( ((uint64_t) &C[(i_stride * i + io_stride * io) * (64)]), (uint32_t) &*(int32_t*)((uint64_t)( ((uint32_t)((uint64_t)res)) + ((i) * (1024))/16)), (16), (16) );
        gemmini_extended_mvout( ((uint64_t) &C[(i_stride * i + io_stride * io) * (64) + 16]), (uint32_t) &*(int32_t*)((uint64_t)( ((uint32_t)((uint64_t)res)) + ((i) * (1024) + 256)/16)), (16), (16) );
        gemmini_extended_mvout( ((uint64_t) &C[(i_stride * i + io_stride * io) * (64) + 32]), (uint32_t) &*(int32_t*)((uint64_t)( ((uint32_t)((uint64_t)res)) + ((i) * (1024) + (2) * (256))/16)), (16), (16) );
        gemmini_extended_mvout( ((uint64_t) &C[(i_stride * i + io_stride * io) * (64) + 48]), (uint32_t) &*(int32_t*)((uint64_t)( ((uint32_t)((uint64_t)res)) + ((i) * (1024) + (3) * (256))/16)), (16), (16) );
        }
    }
  }
}
...
int main() {
  ...
  static elem_t A[12544][256] row_align(1);
  static elem_t B[256][64] row_align(1);
  static elem_t C[12544][64] row_align(1);
  matmul_Context *ctxt;
  float c_scale = (float) ACC_SCALE_IDENTITY;

  static elem_t gold[12544][64] row_align(1);
...
#if CHECK_RESULT == 1
  tiled_matmul_auto(12544, 64, 256,
  ...
  #else
  print_matrix("A", A, 12544, 256);
  print_matrix("B", B, 256, 64);
  print_matrix("C", C, 12544, 64);
  #endif


#if CHECK_RESULT == 1
    if(full_is_equal(gold, C, 12544, 64)){
      #if OUTPUT
      printf("Success\n");
      #endif
    }
